...
void codec2_encode_350(struct CODEC2 *c2, unsigned char * bits, short speech[]);
void codec2_decode_350(struct CODEC2 *c2, short speech[], const unsigned char * bits);
...
struct CODEC2 * codec2_create(int mode)
{
    ...
    if (false == ( CODEC2_MODE_ACTIVE(CODEC2_MODE_3200, mode) 
		   ...
		   || CODEC2_MODE_ACTIVE(CODEC2_MODE_350, mode)
		) ) 
    {
			  printf("not mode\r\n");
        return NULL;
    ...
    c2 = (struct CODEC2*)pvPortMalloc(sizeof(struct CODEC2));
    if (c2 == NULL){
			printf("c2 == null\r\n");
			return NULL;
		}
    ...
    c2->Pn = (float*)pvPortMalloc(2*n_samp*sizeof(float));
    if (c2->Pn == NULL) {
	return NULL;
    }
    c2->Sn_ = (float*)pvPortMalloc(2*n_samp*sizeof(float));
    if (c2->Sn_ == NULL) {
        FREE(c2->Pn);
	return NULL;
    }
    c2->w = (float*)pvPortMalloc(m_pitch*sizeof(float));
    if (c2->w == NULL) {
        FREE(c2->Pn);
        FREE(c2->Sn_);
	return NULL;
    }
    c2->Sn = (float*)pvPortMalloc(m_pitch*sizeof(float));
    if (c2->Sn == NULL) {
        FREE(c2->Pn);
        FREE(c2->Sn_);
        FREE(c2->w);
    ...
    c2->bpf_buf = (float*)pvPortMalloc(sizeof(float)*(BPF_N+4*c2->n_samp));
    ...
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_700C, mode)) {
    ...
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_450PWB, mode)) {
    ...
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_350, mode)) {
        n2_mel_sample_freqs_kHz(c2->n2_rate_K_sample_freqs_kHz, NEWAMP2_K);
        int k;
        for(k=0; k<NEWAMP2_16K_K; k++) {
            c2->n2_pwb_prev_rate_K_vec_[k] = 0.0;
    ...
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_350, mode))
    {
	c2->encode = codec2_encode_350;
	c2->decode = codec2_decode_350;
    }
    return c2;
}
...
void codec2_destroy(struct CODEC2 *c2)
{
    ...
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_350, c2->mode)) {
        codec2_fft_free(c2->phase_fft_fwd_cfg);
        codec2_fft_free(c2->phase_fft_inv_cfg);
    }
    FREE(c2->Pn);
...
int codec2_samples_per_frame(struct CODEC2 *c2) {
	  printf("codec2_samples_per_frame_mode=%d\r\n",c2->mode);
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_3200, c2->mode))
			return 160;
    if ( CODEC2_MODE_ACTIVE(CODEC2_MODE_2400, c2->mode))
			return 160;
...
/*---------------------------------------------------------------------------*\
  ...
  FUNCTION....: codec2_encode_350
  AUTHOR......: Thomas Kurin and Stefan Erhardt, Xu Ruijun
  INSTITUTE...:	Institute for Electronics Engineering, University of Erlangen-Nuremberg
  DATE CREATED: July 2018
  ...
  frame 1: nothing
  frame 2: nothing
  frame 3: 7 bit 1 stage VQ, 3 bits energy, 
           4 bit scalar Wo/voicing/plosive. No spare bits.
  ...
  The bit allocation is:
    ...
    Parameter                      frames 1-3   frame 4   Total
    -----------------------------------------------------------
    Harmonic magnitudes (rate k VQ)     0          7         7 
    Energy                              0          3         3
    log Wo/voicing/plosive              0          4         4
    TOTAL                               0         16        16
...
void codec2_encode_350(struct CODEC2 *c2, unsigned char * bits, short speech[])
{
    ...
    //printf("%i,%i,%i\n", indexes[0], indexes[2], indexes[3]);
    pack_natural_or_gray(bits, &nbit, indexes[0], 7, 0);
    //pack_natural_or_gray(bits, &nbit, indexes[1], 9, 0);
    pack_natural_or_gray(bits, &nbit, indexes[2], 3, 0);
    pack_natural_or_gray(bits, &nbit, indexes[3], 4, 0);
    //printf("%i,%i,%i\n",indexes[0],indexes[2],indexes[3]);
    assert(nbit == (unsigned)codec2_bits_per_frame(c2));
}
...
void codec2_decode_350(struct CODEC2 *c2, short speech[], const unsigned char * bits)
{
    ...
    indexes[0] = unpack_natural_or_gray(bits, &nbit, 7, 0);
    //indexes[1] = unpack_natural_or_gray(bits, &nbit, 9, 0);
    indexes[2] = unpack_natural_or_gray(bits, &nbit, 3, 0);
    indexes[3] = unpack_natural_or_gray(bits, &nbit, 4, 0);
...
float *codec2_enable_user_ratek(struct CODEC2 *codec2_state, int *K) {
    codec2_state->user_rate_K_vec_no_mean_ = (float*)pvPortMalloc(sizeof(float)*NEWAMP1_K);
    *K = NEWAMP1_K;
