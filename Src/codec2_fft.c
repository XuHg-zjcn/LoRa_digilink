...
#else
    retval = pvPortMalloc(sizeof(codec2_fft_struct));
    retval->inverse  = inverse_fft;
    switch(nfft)
    {
		case 64:
        retval->instance = &arm_cfft_sR_f32_len64;
        break;
    case 128:
...
#else
    retval = pvPortMalloc(sizeof(codec2_fftr_struct));
    retval->inverse  = inverse_fft;
    retval->instance = pvPortMalloc(sizeof(arm_rfft_fast_instance_f32));
