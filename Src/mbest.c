...
struct MBEST *mbest_create(int entries) {
    ...
    assert(entries > 0);
    mbest = (struct MBEST *)pvPortMalloc(sizeof(struct MBEST));
    assert(mbest != NULL);

    mbest->entries = entries;
    mbest->list = (struct MBEST_LIST *)pvPortMalloc(entries*sizeof(struct MBEST_LIST));
...
void mbest_search_W25Qxx(
		  uint32_t W25Qxx_cb_ad,     /* VQ codebook to search         */
		  float         vec[],  /* target vector                 */
		  float         w[],    /* weighting vector              */
		  int           k,      /* dimension of vector           */
		  int           m,      /* number on entries in codebook */
...
)
{
   ...
   for(j=0; j<m; j++) {
	e = 0.0;
	for(i=0; i<k; i++) {
	    diff = read_afloat(W25Qxx_cb_ad+j*k+i)-vec[i];
	    e += diff*w[i]*diff*w[i];
	}
	index[0] = j;
	mbest_insert(mbest, index, e);
...
void mbest_search450_W25Qxx(uint32_t W25Qxx_cb_ad, float vec[], float w[], int k,int shorterK, int m, struct MBEST *mbest, int index[])
{
    float   e;
    int     i,j;
    float   diff;
