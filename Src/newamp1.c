...
void resample_const_rate_f(C2CONST *c2const, MODEL *model, float rate_K_vec[], float rate_K_sample_freqs_kHz[], int K)
{
    ...
    for(m=1; m<=model->L; m++) {
        ...
        rate_L_sample_freqs_kHz[m] = m*model->Wo*(c2const->Fs/2000.0)/M_PI;//model->Wo,c2const->Fs
...
float rate_K_mbest_encode(int *indexes, float *x, float *xq, int ndim, int mbest_entries)
{
  ...
  mbest_search(newamp1vq_cb0, x, w, ndim, 512, mbest_stage1, index);
  ...
  for (j=0; j<mbest_entries; j++) {
      ...
      for(i=0; i<ndim; i++)
	  target[i] = x[i] - read_afloat(newamp1vq_cb0+ndim*n1+i);
      mbest_search_W25Qxx(newamp1vq_cb1, target, w, ndim, 512, mbest_stage2, index);
  }
  ...
  mse = 0.0;
  for (i=0;i<ndim;i++) {
      tmp = read_afloat(newamp1vq_cb0+ndim*n1+i) + read_afloat(newamp1vq_cb1+ndim*n2+i);
      mse += (x[i]-tmp)*(x[i]-tmp);
      xq[i] = tmp;
  }
...
void resample_rate_L(C2CONST *c2const, MODEL *model, float rate_K_vec[], float rate_K_sample_freqs_kHz[], int K)
{
   ...
   for(m=1; m<=model->L; m++) {
       ...
       // printf("m: %d f: %f AdB: %f A: %f\n", m, rate_L_sample_freqs_kHz[m], AmdB[m], newmodel->A[m]);
   }
}
                                   ...
                                   int post_filter_en)
{
    ...
    if (user_rate_K_vec_no_mean_ == NULL) {
        /* normal operation */
        for(k=0; k<K; k++) {
            rate_K_vec_no_mean_[k] = read_afloat(newamp1vq_cb0+K*n1+k) + read_afloat(newamp1vq_cb1+K*n2+k);
        }
    } else {
