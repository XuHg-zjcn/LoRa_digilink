...
void n2_rate_K_mbest_encode(int *indexes, float *x, float *xq, int ndim, int VQ_nbit)//ndim=41
{
  int i, n1;
  ...
  /* Stage 1 */
  //printf("%i\n",newamp2vq_cb[0].m);//NEWAMP2_K=29 newamp2vq_cb[0].m=500
	if(VQ_nbit >= 9) {
		VQ_n = 500;
	}
	else {
  ...
  mbest_search450(codebook1, x, w, ndim,NEWAMP2_K, VQ_n, mbest_stage1, index);
                              ...
                              )
{
    ...
    *mean = sum/K;
    ...
    n2_rate_K_mbest_encode(indexes, rate_K_vec_no_mean, rate_K_vec_no_mean_, NEWAMP2_16K_K+1, VQ_nbit);
    ...
    if (model->voiced) {
        int index = encode_log_Wo(c2const, model->Wo, log_Wo_nbit);
        if (index == 0) {
            index = 1;
        }
        if (index == (1<<log_Wo_nbit)-1) {
            index = (1<<log_Wo_nbit)-2;
        }
        indexes[3] = index;
    }
    else {
                              ...
                              int log_Wo_nbit)
{
	...
	//Voiced with Wo
    if (indexes[3]>0 && indexes[3]<(1<<log_Wo_nbit)-1) {
        Wo_right = decode_log_Wo(c2const, indexes[3], log_Wo_nbit);
        voicing_right = 1;
    }
    //Unvoiced
    else if(indexes[3] == 0){
